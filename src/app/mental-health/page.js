...
import { Brain, Smile, Meh, Frown, Wind, BookOpen, Phone, Heart, Sun, Star } from 'lucide-react';
...
const MOODS = [
  { label: 'Great', emoji: '😄', value: 5, color: '#10b981' },
  { label: 'Good', emoji: '🙂', value: 4, color: '#3b82f6' },
  { label: 'Okay', emoji: '😐', value: 3, color: '#f59e0b' },
  { label: 'Low', emoji: '😔', value: 2, color: '#f97316' },
  { label: 'Struggling', emoji: '😢', value: 1, color: '#ef4444' },
];
      ...
      <main className="pt-16">
        <div className="py-10" style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.08), rgba(59,130,246,0.04))' }}>
          <div className="container text-center">
            <span className="badge badge-purple mb-4 inline-flex"><Brain className="w-3 h-3" /> Mental Wellness</span>
            <h1 className="text-4xl font-black mb-3" style={{ fontFamily: 'var(--font-poppins)', color: 'var(--text-primary)' }}>
              Mental Wellness Center
            </h1>
            <p className="text-lg max-w-xl mx-auto" style={{ color: 'var(--text-secondary)' }}>
              Your mental health matters. Tools to help you track, breathe, reflect, and heal.
            </p>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="stat-card">
                <h2 className="font-black mb-2" style={{ color: 'var(--text-primary)' }}>How are you feeling today?</h2>
                ...
                <div className="flex gap-3 flex-wrap mb-4">
                  {MOODS.map(mood => (
                    <button
                      key={mood.label}
                      onClick={() => { setSelectedMood(mood); toast.success(`Mood logged: ${mood.label}`); }}
                    ...
                    >
                      ...
                      <span className="text-xs font-medium" style={{ color: selectedMood?.value === mood.value ? mood.color : 'var(--text-secondary)' }}>{mood.label}</span>
                    </button>
                  ))}
                </div>
                {selectedMood && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="p-3 rounded-xl" style={{ background: `${selectedMood.color}10`, border: `1px solid ${selectedMood.color}30` }}>
                    <p className="text-sm font-medium" style={{ color: selectedMood.color }}>
                      {selectedMood.emoji} Feeling {selectedMood.label.toLowerCase()} today — logged!
                    </p>
                  </motion.div>
                )}
                ...
                {/* Mood Chart */}
                <div className="mt-5">
                  <p className="text-xs font-semibold mb-3" style={{ color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>This Week's Mood</p>
                  <ResponsiveContainer width="100%" height={100}>
                    <AreaChart data={MOOD_HISTORY} margin={{ top: 5, right: 5, left: -30, bottom: 0 }}>
                      ...
                      <Tooltip contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)', borderRadius: 8, fontSize: 12 }} formatter={v => [MOODS.find(m => m.value === v)?.label || v, 'Mood']} />
              ...
              {/* Breathing Exercise */}
              <div className="stat-card">
                <h2 className="font-black mb-2" style={{ color: 'var(--text-primary)' }}>4-7-8 Breathing Exercise</h2>
                <p className="text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>Reduces anxiety and stress. Breathe in for 4s, hold for 7s, exhale for 8s.</p>
                <div className="flex flex-col items-center">
                  <div className="relative w-40 h-40 mb-6">
                    ...
                    >
                      ...
                      <p className="text-sm font-black" style={{ color: 'var(--text-primary)' }}>{breathingActive ? step.label : 'Ready'}</p>
                      {breathingActive && <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>{step.duration}s</p>}
                    </motion.div>
                  </div>
                  {breathCount > 0 && <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>Completed: {breathCount} cycles</p>}
              ...
              <div className="stat-card">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h2 className="font-black" style={{ color: 'var(--text-primary)' }}>Daily Journal</h2>
                </div>
                <textarea
                  value={journal}
                ...
                />
                <button
                  onClick={() => { toast.success('Journal entry saved!'); setJournal(''); }}
            ...
            <div className="space-y-5">
              ...
              <div className="p-5 rounded-2xl" style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)' }}>
                ...
                <h3 className="font-bold mb-2" style={{ color: 'var(--text-primary)' }}>Need Immediate Help?</h3>
                <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)' }}>You are not alone. Reach out — trained professionals are available 24/7.</p>
                {[
              ...
              <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-primary)' }}>
                ...
                <h3 className="font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Wellness Tips</h3>
                {[
                  '🚶 Take a 10-minute walk outside',
                  '💧 Drink a glass of water now',
                  '📵 Take a 30-min digital break',
                  '🧘 Try 5 minutes of mindfulness',
