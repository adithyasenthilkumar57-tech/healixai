          ...
          <div className="container">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)' }}>All your medical reports and documents in one secure place.</p>
                <div className="flex items-center gap-2 mt-3">
                  <Lock className="w-4 h-4 text-emerald-400" />
                  <p className="text-xs text-emerald-400">HIPAA-compliant · End-to-end encrypted</p>
                </div>
              </div>
        ...
        <div className="container py-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: 'Normal Results', value: records.filter(r => r.status === 'normal').length, color: '#10b981' },
              { label: 'Needs Review', value: records.filter(r => r.status === 'attention').length, color: '#f59e0b' },
              { label: 'Storage Used', value: '17.8 MB', color: '#8b5cf6' },
          ...
          <div className="flex gap-3 mb-4 flex-wrap items-center">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search records..." className="input-base pl-10" />
            </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                >
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <span className={`badge text-[10px] flex-shrink-0 ${rec.status === 'normal' ? 'badge-emerald' : 'badge-yellow'}`}>
                      {rec.status === 'normal' ? 'Normal' : 'Review'}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2 mt-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={e => { e.stopPropagation(); toast.success('Opening report viewer'); }} className="btn btn-sm flex-1">
                      <Eye className="w-3 h-3" /> View
                    </button>
                    <button onClick={e => { e.stopPropagation(); toast.success('Downloading...'); }} className="btn btn-sm flex-1">
            ...
            >
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Upload New Report</p>
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>PDF, PNG, JPG supported</p>
            </motion.label>
          </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Try adjusting your filters or upload a new report</p>
            </div>
          )}
        </div>
      </main>
